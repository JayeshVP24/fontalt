...
import { s as selectRandomItems } from "../../chunks/index3.js";
const fonts = [
  {
    "main": {
      "title": "Circular Std",
...
const load = async ({ fetch }) => {
  selectRandomItems(fonts, 10);
  async function getCss() {
    let cssText = "";
    for (const font of fonts) {
      const onlyFonts = [font.main, font.alternativeOne, font.alternativeTwo, font.alternativeThree];
      for (const onlyFont of onlyFonts) {
        const response = await fetch(`/font?font=${onlyFont.fileName}&text=${onlyFont.title}`);
        if (response.ok) {
          cssText += `
 ${await response.text()} 
`;
